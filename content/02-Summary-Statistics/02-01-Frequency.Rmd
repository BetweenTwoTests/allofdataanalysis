...
```{r 02-01-setup-cache, echo=FALSE, message=FALSE, cache=T}
df_chips <- readRDS("data/chocochip_2019.rds")
```

<script src="js/hideOutput.js"></script>

```{r 02-01-setup, echo=FALSE, message=FALSE,}
library(tidyverse)
library(kableExtra)
```
...
```{r chocochip-freq-table-x, echo=FALSE}
chocochip_unit <- 1
...
```{r chocochip-freq-table-x-echo, eval=FALSE, echo=TRUE}
chocochip_unit <- 1
x <- sort(seq(min(df_chips$Time_1), max(df_chips$Time_1), by = chocochip_unit), decreasing = T)
df_freq <- tibble(x = x)
```
...
```{r chocochip-freq-table-f, echo=FALSE}
df_freq <- as_tibble(
  as.data.frame.table(
...
```{r chocochip-freq-table-f-echo, eval=FALSE, echo=TRUE}
